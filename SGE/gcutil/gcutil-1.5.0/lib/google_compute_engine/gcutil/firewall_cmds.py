  ...
  def ParsePortSpecs(port_spec_strings):
    ...
    for port_spec_string in port_spec_strings:
      ...
      if len(parts) > 2:
        ...
        if parts[0]:
          protocol = FirewallRules.ParseProtocol(parts[0])
        port_string = FirewallRules.ReplacePortNames(parts[1])
      else:
        protocol = FirewallRules.ParseProtocol(parts[0])
...
class AddFirewall(FirewallCommand):
  ...
  def Handle(self, firewall_name):
    ...
    firewall_resource = {
        ...
        'name': self._DenormalizeResourceName(firewall_name),
        'description': self._flags.description,
        'rules': []
        }

    if self._flags.network is not None:
      firewall_resource['network'] = (self.NormalizeResourceName(
          self._flags.project,
          'networks',
          self._flags.network))
...
class GetFirewall(FirewallCommand):
  ...
  def Handle(self, firewall_name):
    ...
    firewall_request = self._firewalls_api.get(
        ...
        firewall=self._DenormalizeResourceName(firewall_name))
...
class ListFirewalls(FirewallCommand):
  ...
  def Handle(self, page_size=None, page_token=None):
    ...
    Returns:
      The result of listing the firewalls.
    """
    firewall_request = self._firewalls_api.list(
