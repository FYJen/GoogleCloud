...
class InstanceCommand(command_base.GoogleComputeCommand):
  ...
  def _BuildAttachedDisk(self, disk_arg):
    ...
    disk = {
        'type': 'PERSISTENT',
        'source': self.NormalizeResourceName(self._project, 'disks',
                                             disk_name),
        'mode': mode,
  ...
  def _CreateDiskFromImageRequest(self, disk_name):
    ...
    disk_resource = {
        ...
        'description': 'Persistent boot disk created from %s.' % (
            self._flags.image),
        'zone': self.NormalizeResourceName(self._project, 'zones',
                                           self._flags.zone),
        }
    source_image_url = self.NormalizeResourceName(
        self._project, 'images', self._flags.image)
    return self._disks_api.insert(project=self._project,
                                  body=disk_resource,
    ...
    if self._flags.network:
      network_interface = {
          'network': self.NormalizeResourceName(
              self._project, 'networks', self._flags.network)
          }
...
class ListInstances(InstanceCommand):
  ...
  def Handle(self, page_size=None, page_token=None):
    ...
    """
    instance_request = self._instances_api.list(
...
class SshInstanceBase(InstanceCommand):
  ...
  def _GetInstanceResource(self, instance_name):
    ...
    request = self._instances_api.get(
        project=self._project, instance=instance_name)
...
class GetSerialPortOutput(InstanceCommand):
  ...
  def __init__(self, name, flag_values):
    super(GetSerialPortOutput, self).__init__(name, flag_values)
