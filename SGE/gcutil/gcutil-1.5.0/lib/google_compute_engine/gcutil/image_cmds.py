...
class AddImage(ImageCommand):
  ...
  def Handle(self, image_name, root_source_tarball):
    ...
    if self._flags.preferred_kernel:
      image_resource['preferredKernel'] = self.NormalizeResourceName(
  ...
  def Handle(self, *image_names):
    ...
    for name in image_names:
      requests.append(self._images_api.delete(
          project=self._project,
          image=self._DenormalizeResourceName(name)))
...
class ListImages(ImageCommand):
  ...
  def Handle(self, page_size=None, page_token=None):
    ...
    """
    image_request = self._images_api.list(
...
class Deprecate(ImageCommand):
  ...
  def _BuildRequest(self, image_name):
    ...
    deprecation_status = {
        ...
        'replacement': self._flags.replacement,
